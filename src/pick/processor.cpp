...
const CubeEyeFrameEntry* find_amplitude_frame(const CubeEyeFrameSet& frame_set) {
    const auto it = std::find_if(frame_set.frames.begin(), frame_set.frames.end(),
                                 [](const CubeEyeFrameEntry& entry) { return entry.spec.type == meere::sensor::FrameType::Amplitude; });
    return it == frame_set.frames.end() ? nullptr : &*it;
}
...
std::optional<PickDetectionResult::ObjectPosition> estimate_object_position(const catcheye::BoundingBox& box,
    ...
    const bool cubeeye_detector_grid = camera_frame.width == width && camera_frame.height == height;
    const float offset_u = cubeeye_detector_grid ? 0.0F : RGB_TO_CUBEEYE_OFFSET_U;
    const float offset_v = cubeeye_detector_grid ? 0.0F : RGB_TO_CUBEEYE_OFFSET_V;
    const float center_x = box.x + (box.width * 0.5F);
    const float center_y = box.y + (box.height * 0.5F);
    const float pointcloud_u = std::clamp((center_x / static_cast<float>(camera_frame.width)) + offset_u, 0.0F, 1.0F);
    const float pointcloud_v = std::clamp((center_y / static_cast<float>(camera_frame.height)) + offset_v, 0.0F, 1.0F);
    ...
    const float left_u = std::clamp((box.x / static_cast<float>(camera_frame.width)) + offset_u, 0.0F, 1.0F);
    const float top_v = std::clamp((box.y / static_cast<float>(camera_frame.height)) + offset_v, 0.0F, 1.0F);
    const float right_u = std::clamp(((box.x + box.width) / static_cast<float>(camera_frame.width)) + offset_u, 0.0F, 1.0F);
    const float bottom_v = std::clamp(((box.y + box.height) / static_cast<float>(camera_frame.height)) + offset_v, 0.0F, 1.0F);
...
catcheye::input::Frame cubeeye_amplitude_detector_frame(const CubeEyeFrameSet& cubeeye_frames) {
    const CubeEyeFrameEntry* entry = find_amplitude_frame(cubeeye_frames);
    if (entry == nullptr) {
        throw std::runtime_error("CubeEye amplitude frame is required for amplitude detection");
    }
    if (entry->frame->frameDataType() != meere::sensor::DataType::U16) {
        throw std::runtime_error("CubeEye amplitude frame is not U16");
    }

    const auto basic = meere::sensor::frame_cast_basic16u(entry->frame);
    if (!basic || !basic->frameData() || basic->frameData()->empty()) {
        throw std::runtime_error("CubeEye amplitude frame data is empty");
    }

    const int width = basic->frameWidth();
    const int height = basic->frameHeight();
    if (width <= 0 || height <= 0) {
        throw std::runtime_error("CubeEye amplitude frame has invalid dimensions");
    }

    const auto* data = basic->frameData()->data();
    cv::Mat raw(height, width, CV_16UC1, const_cast<meere::sensor::int16u*>(data));
    cv::Mat gray;
    cv::normalize(raw, gray, 0, 255, cv::NORM_MINMAX, CV_8UC1);
    cv::Mat bgr;
    ...
    frame.timestamp = static_cast<catcheye::input::Timestamp>(entry->frame->timestamp());
