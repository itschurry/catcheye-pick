...
{
    return finite_transform(config.r1) && finite_transform(config.r2) && std::isfinite(config.min_confidence) &&
...
RobotCalibrationConfig load_robot_calibration_config(const std::string& path)
{
    ...
    parse_json_float_field(json, "min_confidence", config.min_confidence);
