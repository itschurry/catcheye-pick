...
- `--pointcloud-downsample <stride>`: pointcloud 송출 downsample 간격을 지정한다. 기본값은 `2`다.
...
CubeEye depth/amplitude/pointcloud:

```bash
./bin/catcheye-pick --viewer-only --ws --cubeeye-frames depth,amplitude,pointcloud --pointcloud-downsample 2
```
