...
exec "$CATCHEYE_PICK_PATH/bin/catcheye-pick" \
  ...
  --depth-pipeline "souphttpsrc location=http://$ISAACSIM_HOST:$ISAACSIM_PORT/api/camera/depth is-live=true do-timestamp=true ! multipartdemux ! jpegdec ! videoconvert ! video/x-raw,format=NV12,width=1280,height=720"
